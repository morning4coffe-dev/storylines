...
public sealed partial class ChapterTextBox : UserControl
{
    ...
    private readonly ChapterTextBoxViewModel _viewModel;

    public ChapterTextBoxViewModel ViewModel => _viewModel;

    public bool dialoguesOn
    {
        get => _viewModel.DialoguesEnabled;
        set => _viewModel.DialoguesEnabled = value;
    }
    ...
    private bool selectedTextIsBold
    {
        get => _viewModel.IsBold;
        set => _viewModel.IsBold = value;
    }
    private bool selectedTextIsItalic
    {
        get => _viewModel.IsItalic;
        set => _viewModel.IsItalic = value;
    }
    private bool selectedTextIsUnderlined
    {
        get => _viewModel.IsUnderlined;
        set => _viewModel.IsUnderlined = value;
    }
    private bool selectedTextIsStriked
    {
        get => _viewModel.IsStrikethrough;
        set => _viewModel.IsStrikethrough = value;
    }
    private bool searchingInTextBox
    {
        get => _viewModel.IsSearchActive;
        set => _viewModel.IsSearchActive = value;
    }
    private int _chapterLocationCaptureSuppressionDepth;
    private string _loadedChapterToken
    {
        get => _viewModel.LoadedChapterToken;
        set => _viewModel.LoadedChapterToken = value;
    }
    ...
    {
        get => _viewModel.IsTypewriterModeActive;
        set
        {
            _viewModel.IsTypewriterModeActive = value;
    ...
    {
        if (!_viewModel.IsTypewriterModeActive || textBoxScrollViewer is null || textBox is null)
            return;
    ...
    public ChapterTextBox()
    {
        ...
        _viewModel = App.GetService<ChapterTextBoxViewModel>();

        dialoguePopupList.ItemsSource = _viewModel.DialoguePopupCharacters;
        ...
        // Wire VM events to view operations
        _viewModel.FormatToggleRequested += OnFormatToggleRequested;
        _viewModel.TextChangedForView += OnViewModelTextChanged;
    ...
    private void OnFormatToggleRequested(string formatType)
    {
        switch (formatType)
        {
            case "Bold": BoldChapterTextBox(); break;
            case "Italic": ItalicChapterTextBox(); break;
            case "Underline": UnderlineChapterTextBox(); break;
            case "Strikethrough": StrikethroughChapterTextBox(); break;
            case "Highlighter": MarkTextBackground(); break;
        }
    }

    private void OnViewModelTextChanged()
    {
    ...
    private void OnTextBox_TextChanged(object sender, RoutedEventArgs e)
    {
        ...
        if (chapter is not null)
        {
            ...
            _viewModel.OnTextChanged(txt, plain, chapter);
        }
    ...
    private void OnTextBox_SelectionChanging(RichEditBox sender, RichEditBoxSelectionChangingEventArgs args)
    {
        ...
        if (_viewModel.IsTypewriterModeActive)
            DispatcherQueue.TryEnqueue(Microsoft.UI.Dispatching.DispatcherQueuePriority.Low, CenterCaretInViewport);
    }
    ...
    private void OnTextBoxScrollViewer_ViewChanged(object sender, ScrollViewerViewChangedEventArgs e)
    {
        ...
        if (_viewModel.IsTypewriterModeActive && !(_textEditor?.IsProgrammaticChangeInProgress == true))
            DispatcherQueue.TryEnqueue(Microsoft.UI.Dispatching.DispatcherQueuePriority.Low, CenterCaretInViewport);
    }
    ...
    {
        if (!_viewModel.IsTypewriterModeActive || textBox?.Document?.Selection is null)
            return false;
    ...
    private void SearchBoxHighlightMatches(string textToFind)
    {
        ...
        // WinUI RichEditBox rejects colors with Alpha = 0 — force fully opaque.
        if (highlightForegroundColor.A == 0)
            highlightForegroundColor.A = 255;

        ITextRange searchRange = textBox.Document.GetRange(0, 0);
        while (searchRange.FindText(textToFind, TextConstants.MaxUnitCount, FindOptions.None) > 0)
        {
            try { searchRange.CharacterFormat.ForegroundColor = highlightForegroundColor; }
    ...
    private void SearchBoxRemoveHighlights()
    {
        ...
        while (range.MoveStart(TextRangeUnit.Character, 1) > 0)
        {
            ...
            catch (ArgumentException) { /* range at document boundary — safe to skip */ }
        }
    }
    ...
    private void SearchReplaceHighlight()
    {
        ...
        {
            try { searchRange.CharacterFormat.ForegroundColor = highlightColor; }
            catch (ArgumentException) { break; }
            matchCount++;
        }
    ...
    {
        _viewModel.DialoguePopupEnteredViaKey = enteredViaKey;
        ...
        _viewModel.RefreshDialoguePopupCharacters();
        ...
        if (_viewModel.DialoguePopupCharacters.Count > 0)
    ...
    {
        if (_viewModel.DialoguePopupCharacters.Count == 0) return;

        int currentIndex = dialoguePopupList.SelectedIndex;
        int newIndex = currentIndex + direction;

        if (newIndex < 0) newIndex = _viewModel.DialoguePopupCharacters.Count - 1;
        else if (newIndex >= _viewModel.DialoguePopupCharacters.Count) newIndex = 0;
    ...
    private void InsertDialogue(Character character)
    {
        ...
        if (_viewModel.DialoguePopupEnteredViaKey)
        {
        ...
        _viewModel.RememberRecentCharacter(character);
        dialoguePopup.IsOpen = false;
    }
    ...
    {
        _viewModel.UpdateFormattingState(selectedTextIsBold, selectedTextIsItalic, selectedTextIsUnderlined, selectedTextIsStriked);
    }
    ...
    {
        return _viewModel.GetLoadedChapter();
    }
    #endregion 
    #endregion
    ...
    private void OnTextBox_PointerWheelChanged(object sender, PointerRoutedEventArgs e)
    {
        ...
        if (_viewModel.IsTypewriterModeActive && (ctrlState & Windows.UI.Core.CoreVirtualKeyStates.Down) != Windows.UI.Core.CoreVirtualKeyStates.Down)
        {
            e.Handled = true;
            return;
        }
